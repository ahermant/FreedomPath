...
const Clarity = function () {
    ...
    this.sprite_1 = { x: 51, y: 1, width: 16, height: 14, direction: "left" };
    this.sprite_2 = { x: 35, y: 1, width: 16, height: 14, direction: "left" };
    this.sprite_3 = { x: 1, y: 1, width: 16, height: 14, direction: "right" };
    this.sprite_4 = { x: 19, y: 1, width: 16, height: 14, direction: "right" };
  
    this.sprite = { x: 1, y: 1, width: 16, height: 14, direction: "right" };
    ...
    window.addEventListener('resize', sizeView.bind(this), false);
...
Clarity.prototype.set_initial_state = function (map) {
    ...
    this.player.image.src = "old_man.png";
...
Clarity.prototype.draw_image = function (context, sprite) {
context.drawImage(
    this.player.image,
    sprite.x,
    sprite.y,
...
Clarity.prototype.switch_animation = function (new_sprite, time) {
this.sprite = new_sprite;
this.last_animation_time = time;
};

Clarity.prototype.draw_player = function (context) {
//context.clearRect( this.player.loc.x, this.player.loc.y, this.tile_size, this.tile_size )

const time = Date.now();

if (this.key.right && this.sprite.direction == "left")
    this.switch_animation(this.sprite_3, time);
else if (this.key.left && this.sprite.direction == "right")
    this.switch_animation(this.sprite_1, time);
else if (
    this.key.right &&
    this.sprite.direction == "right" &&
    time > this.last_animation_time + this.animation_speed
)
    this.sprite == this.sprite_3
    ? this.switch_animation(this.sprite_4, time)
    : this.switch_animation(this.sprite_3, time);
else if (
    this.key.left &&
    ...
    ? this.switch_animation(this.sprite_2, time)
    : this.switch_animation(this.sprite_1, time);

this.player.image.onload = this.draw_image(context, this.sprite);
